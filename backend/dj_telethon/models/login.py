...
class LoginQueryset(models.QuerySet):
    ...
    def have_to_login(self):
        return self.filter(Q(code__isnull=False) | Q(bot_token__isnull=False)).distinct(
            "client_session"
        )
...
class Login(models.Model):
    ...
    have_to_send_code = models.BooleanField(
        default=True, verbose_name=_("Have to send code")
    )

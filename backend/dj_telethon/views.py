...
def send_code_request_view(request):
    ...
    if not phone_number or not client_session_name:
        return JsonResponse(
            {"error": "Missing phone number or client session name"}, status=400
        )
    client_session, is_created = ClientSession.objects.get_or_create(
        name=client_session_name
    )

    if client_session.login_status != LoginStatus.LOGIN_REQUIRED:
        return JsonResponse(
            {"error": "Client session is not in login required status"}, status=400
        )

    if Login.objects.filter(client_session=client_session).have_to_send_code().exists():
        return JsonResponse(
            {"error": "Client session is already waiting for send code."}, status=400
        )
...
def login_user_view(request):
    ...
    if not phone_number or not client_session_name or not code:
        return JsonResponse(
            {"error": "Missing phone number or client session name or code"}, status=400
        )
    try:
    ...
    if (
        login := Login.objects.filter(
            have_to_send_code=False, client_session=client_session
        )
        .order_by("-created_at")
        .first()
    ):
...
def login_bot_view(request):
    ...
    if not bot_token or not client_session_name:
        return JsonResponse(
            {"error": "Missing bot token or client session name or code"}, status=400
        )
    ...
    login = Login.objects.create(
        client_session=client_session, bot_token=bot_token, have_to_send_code=False
    )
    return JsonResponse({"login_id": login.id}, status=200)
